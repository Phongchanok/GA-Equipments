...
const schema = z.object({ code: z.string().min(1), name: z.string().min(1), type: z.string().min(1), location: z.string().min(1), status: z.enum(['ปกติ', 'ผิดปกติ', 'รอตรวจสอบ']).optional() });
...
const schema = z.object({ name: z.string().min(1).optional(), type: z.string().min(1).optional(), location: z.string().min(1).optional(), status: z.enum(['ปกติ', 'ผิดปกติ', 'รอตรวจสอบ']).optional() });
const patch = schema.parse(req.body);
const eq = await Equipment.findOneAndUpdate({ code: req.params.code }, { $set: patch }, { new: true });
if (!eq) return res.status(404).json({ error: 'ไม่พบอุปกรณ์' });
res.json({ equipment: eq });
});
...
const created = await Inspection.create({ equipmentCode: req.params.code, ...data, date: data.date ? new Date(data.date) : new Date() });
...
const days = settings?.inspectionIntervals?.get(eq.type) || 30;
